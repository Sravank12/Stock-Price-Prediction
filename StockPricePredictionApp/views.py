...
def createDic():
    ...
    df2['Name'] = df2['Name'].apply(conv)

    for idx in range(len(df2)):
        name = str(df2.loc[idx, 'Name'])
        symbol = str(df2.loc[idx, 'Symbol'])
        CACHE[name] = symbol

def getSymbol(search):
    global CACHE
    createDic()
...
def explainable(request):
    return render(request,'explainable.html')
def home(request):
    global df
    context={"flag":False}
    if request.method == 'POST':
        stocks = request.POST['company1']
...
def predict(request):
    ...
    if request.method == 'POST':
        ...
        bitcoin = yf.Ticker(stocks)
        if start_date == '2000-04-01' and close_date == '2022-08-13':
            df = bitcoin.history(period="max")
        ...
        model.fit(x_train, y_train, batch_size=512, epochs=5,shuffle=True, validation_split=0.05, callbacks = [checkpoint,stop])
        model.load_weights("./")
...
def predict1(request):
    ...
    return render(request,'predict1.html',context)
...
def all_stocks(request):
    ...
    if request.method == 'POST':
        ...
        print(search)
        all_data = pd.read_csv("C://Users//DELL//OneDrive//Desktop//btp//Stock_Price_Predicton-main//Stock_Price_Predicton-main//StockPricePrediction//all_stocks.csv")
        ...
        for symbol in [search]:
            try:
                ...
                df=bitcoin.history(start=str((date.today() - timedelta(days=13)).strftime("%Y-%m-%d")) ,end=str((date.today() - timedelta(days=5)).strftime("%Y-%m-%d")), actions=False)
                print(df)
...
def all_stocks1(request):
    ...
    if request.method == 'POST':
        ...
        print()
        print(df)
        return render(request,'all_stocks1.html',context) 
    all_data = pd.read_csv('C://Users//DELL//OneDrive//Desktop//btp//Stock_Price_Predicton-main//Stock_Price_Predicton-main//StockPricePrediction//all_stocks.csv')
